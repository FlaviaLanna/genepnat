library(Geneland)
setwd("~/MegaSync/Doutorado/Dados/@Filogeografia/Geneland/nuclear+mt/nuc+Cytb686")
#### Files to be used

geno.temp <- read.table("Cytb_geneland_532.txt",na.string="-999")
geno <- as.matrix(geno.temp)

coord.temp <- read.table("coordinates.Cytb532.txt",na.string='-999')
coord <- as.matrix(coord.temp)

#mt.temp <- read.table('Geneland_Balai_mtDNA_input.txt', na.string='-999')  ###desmarcar quando usar mitocondrial
...
maindir <- "~/MegaSync/Doutorado/Dados/@Filogeografia/Geneland/Cytb532/result" ##só nuclear
#maindir <- "/Users/felipemedeiros/Desktop/Ischnocnema/@Geneland/results10numtDNA" ##nuclear e mit


nrun <- 10        ### number of runs ### obs: aumentar pra no minimo 10
burnin <- 200     ### burnin ####aumentar pra 200
chain <- 5000000  ### number of steps in chain ###aumentar pra 5000000
freq <-      5000 ### sampling frequency 5000
...
for(irun in 1:nrun)
{
  ...
  MCMC(coordinates=coord,
       ...
       rate.max=153, #número de individuos
